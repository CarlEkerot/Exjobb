...
parametrized to capture $80\%$ of the variance. This reduces the
...
number of bits that match against a definition for the current protocol. The
definitions for the different protocols were constructed from openly available
documentation in \citetalias{droms97}, \citetalias{sollins92},
\citetalias{mockapetris87}, \citetalias{schulzrinne03} and
\citetalias{microsoft13}. We only do these in-depth measurements for the header
part of each protocol but our analysis yields similar results within clusters
as well.

For the definitions we quantified a number of combinations of field type and
scope into symbols. We then annotated each bit of the header with these
symbols. A summary of the symbols can be seen in table~\ref{tab:symbols} and
the annotated header definitions can be seen in figure~\ref{fig:dnsdef} to
figure~\ref{fig:dhcpdef}. The resulting accuracy is shown in
...
DNS has a variable number of queries or answers. How the existence of a variable
...
Explain how linear regression and support vector machines could be useful
tools to infer field types, with the assumption that the test set is
good enough.
