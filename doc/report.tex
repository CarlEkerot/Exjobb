...
eliminates the need for an explicit $\varepsilon$ parameter. The results is
an algorithm which is less sensitive to user-specified parameters, and is able
to find clusters with varying densities. One important *** of OPTICS is that
it does not generate actual clusters. Instead, it generates an \emph{ordering}
which reveals density-based structure in the input data. The OPTICS algorithm
extends DBSCAN with the definitions \emph{core-distance} and
\emph{reachability-distance}.
\\[0.5cm]
...
% Få in att det är som att köra DBSCAN med alla möjliga eps-värden
...
%
{
 ...
 Beskriv core objects, core-distances, reachability-distances
