...
The reachability-distance of a sample $p$ is the smallest distance,
$\varepsilon$, which is needed for $p$ to be directly density-reachable to a
core point $q$.
...
global scope, cluster scope, stream and connection scope. We define the global
scope to cover the fields that all messages have in common e.g. the protocol
header. The cluster scope cover fields that are specific for a type. The
connection scope is a subscope to the global and cluster scopes, and cover
...
\begin{figure}[h]
    ...
    \caption{Performance of the OPTICS algorithm for a various number of
        SMB messages.}
\end{figure}
...
\begin{figure}[h]
    ...
    \caption{Performance of the OPTICS algorithm followed by the type
        distinguisher clustering for a various number of SMB messages.}
\end{figure}
