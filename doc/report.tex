...
\subsection{RANSAC}
Random Sample Consensus (RANSAC) \cite{fischler81} is a \emph{robust} method
...

{
    ...
    \begin{algorithm}[H]
        \DontPrintSemicolon
        \BlankLine
        \KwIn{$data$, $model$, $n$, $min\_inliers$, $iterations$, $\varepsilon$}
        \KwOut{$best\_params$}
        \BlankLine
...
relationship. In some clustering algorithms such as K-means \cite{macqueen67},
