...
high similarity have incommon. In order to find these bytes we use an approach
...
\begin{enumerate}
    ...
    We introduce two criterions when considering a byte as a type distinguisher.
...
\begin{figure}[h]
    ...
    \captionsetup{width=0.8\textwidth}
    \caption{Byte value distribution for hte 3rd byte in the DNS header.
...
certain byte may assume. This is decided in the parameter $MaxNumVals$. If
a byte at some position assumes $x$ values where $2 \le x \le MaxNumVals$,
we classify the byte as a flag.
