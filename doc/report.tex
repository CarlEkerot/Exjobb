...
\begin{figure}[t]
    ...
    \begin{subfigure}[b]{0.55\textwidth}
        ...
        \end{tabular}
        \caption{The alignment matrix for the two sequences ATCGG and AATGCAG.}
        \label{fig:nwmatrix}
    \end{subfigure}
...
\begin{enumerate}
    ...
    \item Backtrack through the matrix from $a_{m,n}$ to build the alignments
...
% Skrivan ågot om övriga parametrar?
...
\begin{enumerate}
    \item For every byte offset we 
\end{enumerate}

For every byte offset from the beginning of all messages up to a ceratin limit
...
Now that we have the possible type distinguishers we rank the type
distinguishers based on \emph{completeness score} as defined by
\citet{rosenberg07}.
...
type specific, the data needs to be aligned.
...
non-mutual property is achieved by doing the following modifications:

\begin{enumerate}
    \item After the first row and the first column has been filled with gap
        penalties also fill the diagonal with matches: $a_{j,j} \gets
        a_{j - 1, j - 1} + C_{S_j,T_j}$ where $1 \le j \le n$
\end{enumerate}
